...
Proxy.prototype.checkService = function(String, String, callback) {
  var l = arguments.length,
      args = Array.prototype.slice.call(arguments, 0, (typeof arguments[l - 1] !== 'function' ? l : l - 1)),
      __cb = callback || arguments[1],
      cb = function(ret) {
        if(ret.err) return __cb(ret);
        try {
          require(ret.ret).getProxy(args[1], function(err, proxy) {
            if(err) return __cb({err: err});
            process.nextTick(function() {
              __cb({ret: proxy});
            })
          });
        } catch(e) {
          __cb('Service proxy not found');
        }
      };
  if(args.length == 1) {
    args.push('local');
...
Proxy.prototype.getService = function(String, String, callback) {
  var l = arguments.length,
      args = Array.prototype.slice.call(arguments, 0, (typeof arguments[l - 1] !== 'function' ? l : l - 1)),
      __cb = callback || arguments[1],
      cb = function(ret) {
        if(ret.err) return __cb(ret);
        try {
          console.log('getService return:', ret.ret);
          var proxy = require(ret.ret).getProxy(args[1]);
          __cb({ret: proxy});
        } catch(e) {
          __cb({err: 'Service proxy not found'});
        }
      };
  if(args.length == 1) {
    args.push('local');
